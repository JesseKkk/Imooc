$(function() {
	...
	var pageSize = 3;
	// 列出商品列表的URL
	var listUrl = '/frontend/listproductsbystore';
	// 默认的页码
	var pageNum = 1;
	// 从地址栏里获取storeId
	...
	var searchDivUrl = '/frontend/liststoredetailpageinfo?storeId=' + storeId;
	...
	// $('#exchangelist').attr('href', '/frontend/awardlist?storeId=' +
	// storeId);
	// 获取本店铺信息以及商品类别信息列表
				...
				.getJSON(
						...
						function(data) {
							if (data.success) {
								...
								$('#store-cover-pic').attr('src', store.storeImg);
								$('#store-update-time').html(
										new Date(store.lastEditTime)
												.Format("yyyy-MM-dd"));
								$('#store-name').html(store.storeName);
								$('#store-desc').html(store.storeDesc);
								$('#store-addr').html(store.storeAddr);
								...
								$('#storedetail-button-div').html(html);
							}
						});
	}
	/**
